...
export function renderServiceStars(value: number) {
  ...
  return (
    <View style={serviceDetailStyles.starsIconRow}>
      {names.map((name, index) => (
        <Ionicons key={`${name}-${index}`} name={name} size={15} color="#f2b400" />
      ))}
    </View>
  );
}

export function formatServiceReviewDate(isoDate: string) {
